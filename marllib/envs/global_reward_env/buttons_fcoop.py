...
class RLlibBUTTONS_FCOOP(RLlibButtons):
    ...
    def step(self, action_dict):
        ...
        s_team = np.full(self.num_agents, -1, dtype=int)
        ...
        for i, agent in enumerate(self.agents):
            ...
            projected_l_dict[i] = list(set(self.envs[agent].reward_machine.get_events()) & set(l))
            ...
            if projected_l_dict[i]:
                for event in projected_l_dict[i]:
                    for j in range(self.num_agents):
                        ...
                        if (event in set(self.envs[self.agents[j]].reward_machine.get_events())) and (not (projected_l_dict[j] == projected_l_dict[i])):
                            projected_l_dict[i] = []


            for e in label: # there really should only be one event in the label provided to an individual agent
